...
def dti(txt_path, save_path, n_min=25, n_dti=20):
    ...
    seq_txts = sorted(Path(txt_path).glob("*.txt"))
    for seq_txt in seq_txts:
        seq_name = seq_txt.name
        print(seq_name)
        ...
        for track_id in tqdm(range(min_id, max_id + 1)):
